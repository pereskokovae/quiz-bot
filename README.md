...
В скрипте `serializer_json.py` есть функция  отвечающая за парсинг данных, она берёт сырые файлы из папки `quiz_questions`, парсит их и формирует единый JSON quiz_questions.json.
Что делает :
 • читает все файлы в quiz_questions/;
 • парсит содержимое (вопрос / ответ(с пояснением) ), очищает текст;
 • собирает словарь и записывает quiz_questions.json.
#### Как запустить
1. Создайте папку quiz_questions с необработанными данными
2. Запустите скрипт командой:
```bash
python serializer_json.py
```
