...
```bash
TG_BOT_API_KEY=токен_вашего_тг_бота
REDIS_URL=redis://:PASSWORD@HOST:PORT/0
VK_BOT_API_KEY=токен_вашего_вк_бота
```
#### Как получить

| Переменная    | Описание      |
| ------------- | ------------- |
| TG_BOT_API_KEY | токен вашего Telegram бота, чтобы его получить, создайте бота в [@BotFather](https://telegram.me/BotFather).  |
| VK_BOT_API_KEY | токен вашего VK бота, полученный в настройках группы во вкладке "Работа с API" --> "Создать ключ доступа".  |
...
В скрипте `quiz_parser.py` есть функция, которая отвечает за загрузку сырого текста вопросов из указанной директории.
